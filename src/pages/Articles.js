...
const Article3 = ({ img, title, date, link }) => {
  return (
    ...
    >
      ...
      <span className="text-primary font-semibold pl-4 dark:text-purple-300 sm:self-start sm:pl-0 xs:text-sm">{date}</span>
    </motion.li>
  );
};
...
const Featuredarticle = ({ img, title, time, summary, link }) => {
  return (
    <li className="col-span-1 w-full p-4 bg-light border border-solid border-dark rounded-2xl dark:border-light dark:bg-dark dark:text-light">
      <Link
        href={link}
        target="_blank"
      ...
      <Link href={link} target="_blank">
        ...
        <span className="text-primary font-semibold dark:text-purple-300">{time}</span>
      </Link>
    </li>
  );
};
